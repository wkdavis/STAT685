...
While renting transportation has been around for decades in rental car companies, such as those often found at airports, it
has also been adapted to the modern "sharing economy" through
bike-sharing programs. These programs, often offered by local governments, allow
...
A key challenge faced by administrators of bike-sharing programs is the efficient
allocation of available bicycles. Bikes must be available where people need them 
and at the time they are needed for the program to be effective. In order to efficiently allocate bikes,
@schuijbroek suggest administrators can
periodically transport bikes from areas of low demand to areas of high demand.
Demand in a bike-sharing system is a function of both time
...
provides suggestions for action and application of the results while highlighting potential areas for future research.

# Literature

The popularity and accessibility of the Seoul bike dataset have resulted in its
use in numerous studies. A majority of these studies have focused on the
...
method, random forest (RF), performed best on a similar bike-sharing dataset when
measured by $R^2$ and RMSE. Both studies further showed
that weather-related variables, such as temperatures and precipitation, were
...
methodologies when predicting demand of a similar nature to the bike-sharing data.
...
bike-sharing system not functioning. There are no other periods where
*BikeCount*=0. Table \ref{tab:var-defs} contains information on the variables in
the dataset, including the variable type.
...
decrease again in the autumn. This heteroscedasticity violates the constant
...
is certainly an effect based on the hour of the day, with demand increasing
...
which is consistent with the idea of heteroscedasticity; the variance increases
...
though the difference does not appear particularly large. There are many
large positive outliers during the weekdays. Finally, demand appears
...
hourly pattern much like the workdays, we can see that there is a difference
between the seasonality of workdays and the seasonality of non-workdays. Most
notably, the troughs in demand appear consistent between the type of days, while 
the peaks are consistently higher for workdays compared to non-workdays. 
This changing seasonality based on the type of days will need to be captured in our model.


Continuing with the impact of seasonality and temperature on demand, we will
next explore covariates included in the dataset.

## Covariates

This dataset includes several covariates to aid in modeling bike demand. 
These covariates are listed in Table 1, along with their definitions. 
The covariates fall into one of two broad categories: weather and social. 
Weather covariates include temperature, humidity, precipitation, and others. 
Social variables capture the impact of calendar-based
...
months and a downward trend that hits a trough in the winter months. Humidity, 
...
methods like ARIMA and modern methods, including the regression-based
...
typical assumptions of parametric methods required for inference: independent 
observations and normally distributed residuals with mean zero and constant variance. 
The assumption of independent observations is obviously violated in time series data 
and most often appears in the form of autocorrelated errors. 
Therefore, the ability to disregard the assumptions
...
(SARIMA) process
instead of the traditional iid normal distribution with mean 0 and constant
variance. This results
in the modified regression equation

\begin{equation}
\label{eq:ols}
y_t=\beta_0+\sum_{j=1}^k\beta_jz_{jt}+\epsilon_t
\end{equation}
...
* $\epsilon_t$ are the regression errors, which are assumed to follow an SARIMA process
as in (\ref{eq:sarima}).
...
\begin{equation}
\label{eq:sarima}
\Phi_P(B^S)\phi(B)\nabla_S^D\nabla^d\epsilon_t=\Theta_Q(B^S)\theta(B)w_t
\end{equation}
...
* $\epsilon_t$ are the errors from \ref{eq:ols}.

as defined by @shumway. SARIMA models of this form are often written
...
```{r acf, fig.cap="Correlograms for $\\nabla^{24}y_t$", fig.height=2}
grid.arrange(
bikets_cv %>%
  mutate(BikeCount = difference(box_cox(BikeCount, l), lag = 24)) %>%
  ACF(BikeCount, lag_max = 24*7) %>%
  autoplot() + labs(title = expression(paste("ACF of ", nabla^24*y[t]))) + theme(axis.title.y = element_blank(), axis.text.x = element_text(size = 5)),
bikets_cv %>%
  mutate(BikeCount = difference(box_cox(BikeCount, l), lag = 24)) %>%
  PACF(BikeCount, lag_max = 24*7) %>%
  autoplot()+ labs(title = expression(paste("PACF of ", nabla^24*y[t]))) + theme(axis.title.y = element_blank(), axis.text.x = element_text(size = 5)),
ncol = 2)
```
...
y_t&=\beta_0+\sum_{j=1}^k\beta_jz_{jt}+s_d(t,m)+s_w(t,n)+\epsilon_t\\
\label{eq:fourierday}
s_d(t,m)&=\sum_{i=1}^m\left[\alpha_i\sin\left(\frac{2\pi it}{24}\right)+\eta_i\cos\left(\frac{2\pi it}{24}\right)\right]\\
\label{eq:fourierweek}
s_w(t,n)&=\sum_{l=1}^n\left[\nu_l\sin\left(\frac{2\pi lt}{168}\right)+\delta_l\cos\left(\frac{2\pi lt}{168}\right)\right]
\end{align}
...
* $s_d(t,m)$ are the $m$ daily seasonality fourier terms with coefficients
$\alpha$ and $\eta$.
* $s_w(t,n)$ are the $n$ weekly seasonality fourier terms with coefficients
$\nu$ and $\delta$.
* $\epsilon_t$ are the model errors, which are assumed to follow a $ARIMA(p,d,q)$
...
likely be biased. The use of covariates may capture some of the patterns in
...
generate point forecasts, which are still valid, albeit biased, even when the errors
...
$SARIMA(1,0,1)\times(5,1,0)_{24}$ for $\epsilon_t$.
...
Finally, we will test a SARIMAX model using two derived
...
we will add a $\text{Temperature}^2$ term to the model. Additionally, 
as shown in Figure \ref{fig:covariate-timeplot}, Rainfall and Snowfall 
are continuous variables with a large number of zero values. We believe it is
...
Unfortunately, Figure \ref{fig:auto-sarima-poly} shows that there is still
...
(non-seasonal) $AR(p)$ and $MA(q)$ terms. As @prophet notes, missing values need not
be manually imputed because this is a curve-fitting exercise rather 
than a generative model. Instead, the curves $g(t)$, $s_d(t,m)$, and
...
recommended by @prophet. We will test a number of different parameter combinations,
$m\in\{1, 2, 5, 10, 20, 50\}$ and $n\in\{3, 5, 10, 20, 50\}$, to see which
combination provides the best model (smallest error).

```{r prophet-cv, fig.cap="MAE for Prophet models with various numbers of (daily and weekly) fourier terms", fig.height=3, fig.width=3}
holidays <- bikets_cv %>%
...
capturing all of the autocorrelations in the data: the strength of the
correlation appears to be a non-linear function of time.
One reason for this
shape in the PACF plot could be the presence of switching seasonality. More specifically, 
the seasonality effects (not just the series itself) may possibly differ on working and non-working 
days. A hand-wavy analysis would say that, given seven days a week, any day is, on "average,"
$7/2=3.5$ days away from any other day.
For weekdays specifically, they are 3.5 days away from a weekend (again, on
average).
Therefore, the trough in the PACF curve occurs between 3 and 4 days
might be due to that lag most frequently correlating weekdays with weekend days,
resulting in a lower correlation than when weekdays are compared with other
...
previously employed techniques (SARIMAX and Prophet) do not allow for changing
seasonality.
...
The terms $E_t$ and $G$ operate together to allow for different hourly seasonality
components for workdays and non-workdays. For example, the effect
(coefficient) of the 8:00 AM hour can be different for a workday and a
non-workday.
...
y_t&=E_t\rho_t+v_t,&&v_t\sim N(0,V)\\
\label{eq:fasster-state}
\rho_t&=G\rho_{t-1}+\omega_t,&&\omega_t\sim N(0,\Omega)
\end{align}

where:

* $y_t$ is the bike demand at time $t$
* $\rho_t$ are the underlying states at time $t$
* $E_t$ are the coefficients at time $t$ for (linearly) combining the states to
produce the response at time $t$ ($y_t$)
* $G$ is a matrix that defines the behavior of each state (season, trend, level,
etc)
* $v_t$ and $\omega_t$ are normally distributed random noise with mean 0.
...
the autoregression in the series, resulting in significantly autocorrelated
...
LSTM is a variety of Recurrent Neural Networks (RNNs). According to @hochreiter_1997_long, the benefit of the Long Short-Term Memory (LSTM) network over other recurrent networks comes from "constant error backpropagation," an improved method of back-propagating the error.

```{r LSTM note, echo=FALSE, fig.cap="The repeating module in an LSTM contains four interaction layers. Images borrowed from \\citet{colah}'s blog.", out.width = '80%'}
knitr::include_graphics("LSTM-chain.png")
knitr::include_graphics("LSTM2-notation.png")
```

The key to LSTMs is the cell state, the horizontal line running through the top of the graph. An LSTM features three gates: input, forget, and output. Gates allow information to pass through optionally. They comprise a Sigmoid neural net layer and a pointwise multiplication operation.
...
For regular RNNs, the derivative of an activation function, such as a sigmoid, will be less than one during backpropagation. Therefore, repeated multiplications of that value against weights will lead to vanishing gradients over time.
...
state passes through unchanged. This is why LSTMs can deal with more intricate
problems than the RNNs by keeping a constant flow of error throughout the
...
\begin{equation}
z_i^* = \frac{z_i-\min\{\vec z\}}{\max\{\vec z\}-\min\{\vec z\}}
\end{equation}
...
The processed dataset was partitioned into a train set and a test set where about 86% 
...
by a training and validation loss that decreases to the point of stability with a
...
## Solve randomness of LSTM results
...
while training is represented. Both training and testing loss decreases smoothly and fairly quickly.  
...
Given that our stated use for these models is forecasting (prediction), when
...
MAE is the preferred error metric for selecting the best forecasting model and
...
\label{eq:mae}
\text{Mean Absolute Error}&=\frac{1}{\mu \iota}\sum_{i=1}^\mu\sum_{j=1}^\iota |e_{ij}|\\
\label{eq:rmse}
\text{Root Mean Squared Error}&=\sqrt{\frac{1}{\mu \iota}\sum_{i=1}^\mu\sum_{j=1}^\iota e_{ij}^2}
\end{align}

Where $\iota=24$ is the number of periods for which bike count is forecast (the
horizon) and $\mu=50$ is the number of iterations (folds) in the 
...
Each forecast is then compared with actual (observed) values to evaluate the forecast.
...
Our evaluation will use the last 50 days of data as the test folds, with
...
In this case, bike demand can be assumed to be seasonal with a daily (24 hours)
period.
Given hourly data, a common-sense baseline is to predict the bike demand ($x_t$) at
...
The baseline SNAIVE model outperformed the
...
within the operational constraints of the bike-sharing management organization
...
could focus on the inclusion of additional predictor variables and more
recent bike usage data. One key deficit of the models presented in this analysis
is that they do not account for spatial variation in demand. That is, what is
...
needed would allow the administrators of the bike-sharing to reallocate available
...
value to the bike-sharing administrators, making it a strong candidate for
subsequent research.

...
```{r arima-fourier, fig.height=4, fig.cap="Residuals for an ARIMAX(2,1,2) model with daily and weekly fourier terms", fig.scap="\\label{fig:acf-arimax}"}
...
(Figure \ref{fig:acf-arimax}) still show significant autocorrelation.
...
```{r auto-sarima, fig.height = 4, fig.cap="Residuals for an $SARIMAX(1,0,1)\\times(5,1,0)_{24}$", fig.scap="\\label{fig:acf-sarimax}"}
...
Figure \ref{fig:acf-sarimax} shows that, while the autocorrelation (and partial autocorrelation)
...
model (Figure \ref{fig:acf-sarimax}) and the Prophet model (Figure \ref{fig:prophet-resid}) is that
...
```{r accuracy-viz, fig.cap="Mean Absolute Error by Model", fig.height=3, fig.width=4, cache = FALSE, fig.scap="\\label{fig:mae-model}"}
combined_acc %>%
...
Table \ref{tab:error-by-model} and Figure \ref{fig:mae-model} report the MAE and RMSE for each model. Perhaps

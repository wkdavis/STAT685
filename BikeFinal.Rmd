...
*BikeCount*=0. Table 1 contains information on the variables in
...
Figure 1 highlights the incredible variation in demand over time.
...
```{r seasonality-weekends, fig.cap="Hourly bike demand by type of day, highlighting differing seasonality between workdays and non-workdays"}
grid.arrange(
bikets[1000:1672,] %>%
...
ggplot(aes(x = Hour, y = BikeCount, color = `Day Type`)) +
  geom_line() +
  theme(legend.position = "top", axis.title.x = element_text(size = 8)) +
  labs(title = "Hourly Bike Demand by Type of Day",
...
```{r covariate-timeplot, fig.cap="Hourly time plots of covariates"}
bikets %>%
...
Time plots of the covariates (Figure 4)
...
```{r corrplot, fig.width=8, fig.cap="Correlation plot of covariates"}
...
The correlation plot highlights a number of important relationships in the data.
Bike count appears moderately linearly correlated with both temperature
($\rho=0.563$) and dewpoint ($\rho=0.400$). However, this is significant
...
ARIMAfourier <- readRDS("ARIMAfourier.RDS")
ARIMAfourier %>%
  residuals() %>%
...
ARIMAauto <- readRDS("ARIMAauto.RDS")
ARIMAauto %>%
  residuals() %>%
  gg_tsdisplay(.resid, plot_type = "partial", lag_max = 24*7)+
...
```{python series-to-supervised-fn}
...
```{python import-packages, include = FALSE}
...
```{python python-load-data, include = FALSE, eval = FALSE}
# load data
bike = r.bike
...
```{python python-data-prep, include = FALSE}
...
```{python lag1-data-prep, include = FALSE}
# convert categorical features to numeric encoding
...
```{python lag1-model, include = FALSE}
# design network
model = Sequential()
...
```{python lag1-loss, fig.cap='Model loss - 1 hour lagged time steps as input', echo = FALSE, fig.height = 4, fig.width = 5}
...
Although in Figure 12 we can see both errors converge fairly fast, the proposed
network is not a good fit since there is a gap between the training error and
test error.

```{python lag1-error, echo = FALSE}
# make a prediction
...
```{python lag1-forecast, fig.cap='Forcasted Bike Rental Demand of one day - 1 hour lagged time steps as input', echo = FALSE, fig.height = 4, fig.width = 5}
...
```{python lag12-data-prep, include = FALSE}
...
```{python lag12-model, include = FALSE}
# design network
model = Sequential()
model.add(LSTM(50, activation = 'relu', input_shape=(train_X.shape[1], train_X.shape[2])))
model.add(Dropout(0.5))
...
```{python lag12-loss, fig.cap='Model loss - 12 hour lagged time steps as input', echo = FALSE, fig.height = 4, fig.width = 5}
# plot history
pyplot.plot(history.history['loss'], label='train')
pyplot.plot(history.history['val_loss'], label='test')
...
The Loss vs Epoch curve is shown in Figure 13 in which the progress of mode
while training is represented. Both training and testing loss decreases in a
smooth fashion fairly quickly.  

```{python lag12-error, echo = FALSE}
# make a prediction
yhat = model.predict(test_X)
test_X = test_X.reshape((test_X.shape[0], n_hours*n_features))
# invert scaling for forecast
...
```{python lag12-forecast, fig.cap='Forcasted Bike Rental Demand of one day - 12 hour lagged time steps as input', echo = FALSE, fig.height = 4, fig.width = 5}
## time steps, every step is one hour (you can easily convert the time step to the actual time index)
...
```{python lag24-data-prep, echo = FALSE}
# specify the number of lag hours
...
```{python lag24-model, include = FALSE}
# design network
model = Sequential()
model.add(LSTM(50, activation = 'relu',input_shape=(train_X.shape[1], train_X.shape[2])))
model.add(Dropout(0.5))
...
```{python lag24-loss, fig.cap='Model loss - 24 hour lagged time steps as input', echo = FALSE, fig.height = 4, fig.width = 5}
# plot history
pyplot.plot(history.history['loss'], label='train')
pyplot.plot(history.history['val_loss'], label='test')
...
```{python lag24-forecast, fig.cap='Forcasted Bike Rental Demand of one day - 24 hour lagged time steps as input', echo = FALSE, fig.height = 4, fig.width = 5}
## time steps, every step is one hour (you can easily convert the time step to the actual time index)
## for a demonstration purpose, I only compare the predictions in 24 hours. 
...
```{r tscv-example, fig.height=3, fig.cap="Visualization of time series cross-validation example"}
per <- 1:20
...
## Time Series
tsmodels_cv <- readRDS("tsmodels_cv.RDS")
tsmodels_acc <- forecast(tsmodels_cv, new_data = fcst_new_data) %>%
...
# 
rfTestOut <- readRDS("rfTestOut.RDS")
# 
...
#                  trControl = myTestControl)
# 
boostTestOut <- readRDS("boostTestOut.RDS")
...
combined_acc %>%
  ...
  bind_rows(tibble::tibble(.model = c("LSTM lag 1", "LSTM lag 12", "LSTM lag 24"),
               MAE = c(py$maeL1, py$maeL12, py$maeL24),
               RMSE = c(py$rmseL1, py$rmseL12, py$rmseL24))) %>%
  rename(Model = .model) %>%
  knitr::kable(caption = "Test Error by Model Type", digits = 0) %>%

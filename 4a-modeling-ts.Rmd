...
bikets_tall %>%
  features(Value, c(unitroot_kpss, unitroot_ndiffs, unitroot_nsdiffs, unitroot_pp, ljung_box))
...
bikearima <- bikets %>%
  ...
  model( arimaACF = ARIMA(box_cox(BikeCount, l) ~ 0 + PDQ(0, 1, 1) + pdq(0, 1, 11)),
         ...
         arimaPACF = ARIMA(box_cox(BikeCount, l), stepwise = FALSE, approximation = FALSE))
  ...
  model(
    prophet = prophet(
      log(BikeCount) ~ season(
        ...
        order = 50,
        name = "weekly"
      ) + holiday(holidays = holidays) + Dewpoint + Humidity + Rainfall + SolarRadiation + Temperature + Visibility + WindSpeed + BikeCountL1 + BikeCountL24 + Seasons + growth())
  )
